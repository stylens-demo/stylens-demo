...
//    _cropAreaView.frame = CGRectMake(s0, 0, 0, 0);
    [self resetCornersOnCropAreaFrameChanged];
    [self resetCropTransparentArea];
...
// Added by Rano.
-(void)resetCropAreaWithCGRect:(CGRect)aRect {
    _cropAreaView.frame = aRect;
    _cropAreaView.hidden = NO;
    [self resetCornersOnCropAreaFrameChanged];
    [self resetCropTransparentArea];
    [self resetMinSpaceIfNeeded];
}
...
- (void)setToCropImage:(UIImage *)toCropImage {
    ...
    // [self resetCropAreaWithCGRect:CGRectNull];
